...
void main() {
	...
	#else
		outgoingLight = refractCol.rgb;
	#endif
	
	vec4 map = texture2D(envMap, uv);
	// gl_FragColor = map;	
	outgoingLight = mix(map.rgb, outgoingLight, .8);

	gl_FragColor = vec4(final.rgb, 1.0);

	float zthreshold = smoothstep(-3., -2., 0.);
	// zthreshold = mix(zthreshold, 1., vBackface);
	float glowThreshold = smoothstep(-2., -1., 0.);
